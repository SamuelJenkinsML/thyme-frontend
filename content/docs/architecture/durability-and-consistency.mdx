...
| `Min` | Minimum of a field in window. *(Not currently recommended — see [Known Limitations](/docs/operations/known-limitations).)* |
| `Max` | Maximum of a field in window. *(Not currently recommended — see [Known Limitations](/docs/operations/known-limitations).)* |
...
- **Synchronous read-after-write within a single millisecond of ingestion.** Aggregations advance per pipeline tick, not per event. Feature freshness depends on the configured batch size and broker throughput (see [Scaling](/docs/operations/scaling)).
