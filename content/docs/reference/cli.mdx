...
| `--query-url URL` | — | `THYME_QUERY_URL` | `http://localhost:8081` | Query-server URL |
...
Direct dataset point lookup — returns a raw row from RocksDB state, skipping the extractor pipeline.
...
6. Built-in defaults (localhost)

### `THYME_FRONTEND_URL`

Set this to the base URL of your Thyme web UI (e.g. `http://localhost:3000` for local dev, your internal URL for infra). When set, query commands print a clickable `Results:` link to the query-run detail page.

```bash
export THYME_FRONTEND_URL=http://localhost:3000
thyme query myproject.features:UserFeatures -e user_42
# ...table...
# Query run: 7b3e4c...
