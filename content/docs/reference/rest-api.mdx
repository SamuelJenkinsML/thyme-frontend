...
| Service | Default URL | Set with |
|---------|-------------|---------|
| Definition service | `http://localhost:8080` | `DEFINITION_SERVICE_URL` |
| Query server | `http://localhost:8081` | `QUERY_SERVER_URL` |

## Authentication

When `THYME_API_KEY` is set on the server, every request (except `/health` and `/metrics`) requires:

```
Authorization: Bearer <api-key>
```

Invalid or missing tokens return `401`. When `THYME_API_KEY` is not set, auth is disabled (local-dev mode). See [Authentication](/docs/reference/authentication) for the full auth model.
...
```bash
# Online query
curl "http://localhost:8081/features?entity_id=user_42&featureset=UserFeatures"

# Point-in-time
curl "http://localhost:8081/features?entity_id=user_42&featureset=UserFeatures&timestamp=2026-04-01T00:00:00Z"
```
...
Re-runs a captured query — useful for reproducing a production result locally or in a debugging context.
...
| `5xx` | Server-side failure — see [Operations → Monitoring](/docs/operations/monitoring) for log paths |
