...
When a UDF fails repeatedly (user exception, timeout, worker crash), the engine doesn't halt. After exhausting retries, the failing batch is routed to a per-dataset dead-letter topic (`<output_dataset>.udf_dead_letter`) so you can inspect it without blocking the pipeline. The engine then continues processing.

To inspect dead-letter contents during a live incident:

```bash
docker exec thyme-redpanda-1 rpk topic consume Order.udf_dead_letter --num 5 --format json
```
