...
Every pipeline tick commits as a single atomic unit: a RocksDB `WriteBatch` (output state, internal accumulator state, input offset) and a transactional Kafka produce. On restart the engine restores state from a checkpoint and replays from the committed offset. No duplicates, no lost events, no partial state.
...
The query server runs the extractor DAG via PyO3 directly inside the Rust process, reading aggregated state from a local RocksDB secondary. No network hop to fetch state. No sidecar. No JVM.
...
You do not manage Kafka consumer groups, RocksDB compaction, partition leasing, checkpoint upload, watermark advancement, or late-event handling. The engine does. Your job is the feature logic; Thyme's job is running it at scale.
...
**Safe schema evolution.** Features carry integer IDs. You can add, rename, and version features without breaking downstream consumers.
