...
| Deployment model | Hosted | Managed SaaS | Managed SaaS | DIY |
...
2. **Sub-millisecond reads on the hot path.** Thyme is designed for online inference: P50 server-side latency is consistently under 1 ms in production benchmarks. Most managed feature stores hit comparable latency only on their paid online-serving tiers.
