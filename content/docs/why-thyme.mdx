...
```bash
# Online
curl "http://.../features?featureset=UserFeatures&uid=user_42"

# Point-in-time (training)
curl "http://.../features?featureset=UserFeatures&uid=user_42&ts=2024-01-15T12:00:00Z"
```
...
Thyme is one of several feature platforms in the streaming-feature space. The closest comparators:

| | **Thyme** | **Tecton** | **Fennel** | **Feast** |
|---|---|---|---|---|
| Architecture | Kappa (single pipeline) | Lambda (batch + streaming) | Lambda internally | Batch-only |
| Online / offline parity | **Structural** (one source of truth) | Reconciled at materialization | Reconciled internally | Manual snapshots |
| Streaming aggregations | Native (`@pipeline` over windows) | Native | Native | Not supported |
| Approximate percentiles | Native (`ApproxPercentile`) | Native (`approx_percentile`) | Not supported | Not supported |
| Derived features (Python) | Query-time extractor, in-process | "On-demand features" | Python transforms | Not supported |
| Temporal joins (SCD-style) | Native (`.join(..., fields=[...])`) | Native | Native | Not supported |
| Data quality / expectations | Built-in (`@expectations`) | External (Great Expectations) | Not supported | Not supported |
| Self-hostable | **Yes** (open source) | Managed only | Managed only | Yes |
| Deployment model | Kubernetes (Helm / Terraform reference) | Managed SaaS | Managed SaaS | DIY |

Three things to call out:

1. **Kappa, not lambda.** Thyme, Tecton, and Fennel all support streaming aggregations, but only Thyme guarantees online/offline parity *structurally* — the same pipeline answers both query types from the same materialized state. Tecton and Fennel reconcile a batch pipeline against a streaming pipeline and bear the operational cost of that reconciliation.

2. **Sub-millisecond reads in the open-source path.** Thyme is designed for the hot path of online inference: P50 server-side latency is consistently under 1 ms in production benchmarks. Most managed feature stores hit comparable latency only on their paid online-serving tiers.

3. **Self-hostable.** You can run Thyme on your own Kubernetes cluster from the open-source repo — see [Operations → Deployment](/docs/operations/deployment). Tecton and Fennel are managed-only.
