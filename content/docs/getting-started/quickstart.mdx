...
```bash
curl "http://localhost:8081/features?featureset=UserFeatures&entity_id=user_42"
```
...
Set `THYME_FRONTEND_URL=http://localhost:3000` (or your deployed URL) and the CLI will print a clickable `Results:` link to the **Query Run** page in the UI, where you can see latency, hit rate, and replay the query.
