...
Each Kinesis shard supports approximately **1 MB/s or 1,000 records/s** for read. To scale beyond that, increase the shard count on the source stream:
...
See [Scaling](/docs/operations/scaling) for end-to-end throughput sizing across pipeline partitions and Kinesis shards.
...
- Backfilling a Kinesis-sourced dataset on commit is not currently supported. See [Known Limitations](/docs/operations/known-limitations).
