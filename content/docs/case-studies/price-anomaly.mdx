...
```bash
make infra && make migrate && make build

cd demos/price_percentile
uv sync
uv run python seed_data.py --postgres
uv run thyme commit features.py

curl "http://localhost:8081/features?entity_id=p_spike&featureset=PriceFeatures"
```

The full demo is available in the [Thyme repository](https://github.com/SamuelJenkinsML/thyme) under `demos/price_percentile/`.
